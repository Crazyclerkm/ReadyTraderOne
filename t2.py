...
class AutoTrader(BaseAutoTrader):
    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        if instrument == Instrument.FUTURE:
            ...
            # the bid/ask id
            if self.bid_id != 0 and new_bid_price not in (self.bid_price, 0) and new_bid_price < midpoint:
                self.send_cancel_order(self.bid_id)
                self.bid_id = 0
                self.bid_count = -1
            if self.ask_id != 0 and new_ask_price not in (self.ask_price, 0) and new_ask_price > midpoint:
                self.send_cancel_order(self.ask_id)
                self.ask_id = 0
                self.ask_count = -1
            ...
            if self.bid_id == 0 and new_bid_price != 0 and new_bid_price < midpoint * 1.05 and self.position < POSITION_LIMIT:
            ...
            if self.ask_id == 0 and new_ask_price != 0 and new_ask_price >= midpoint * 0.95 and self.position > -POSITION_LIMIT:
